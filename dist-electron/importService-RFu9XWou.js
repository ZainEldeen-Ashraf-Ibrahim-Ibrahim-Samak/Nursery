			...
			settingsSheet.eachRow({ includeEmpty: false }, (row, r) => {
				try {
					...
					if (label.includes("حضانة")) {
						if (monthly > 0) setServicePrice("حضانة", "price_monthly", monthly);
					} else if (label.includes("استضافة")) {
						if (monthly > 0) setServicePrice("استضافة", "price_monthly", monthly);
					} else if (label.includes("جلسة")) {
						if (hourly > 0) setServicePrice("جلسة", "price_hourly", hourly);
...
//# sourceMappingURL=importService-RFu9XWou.js.map