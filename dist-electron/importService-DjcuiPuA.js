import { n as getDb } from "./connection-BvJpTvbB.js";
...
async function importFromWorkbook(filePath) {
  ...
  `);
	...
	for (const ws of workbook.worksheets) {
		if (isIgnoredSheet(ws.name) || isChildrenSheet(ws.name) || isSalarySheet(ws.name) || isExpensesSheet(ws.name) || isSettingsSheet(ws.name) || isTargetSheet(ws.name) || isDashboardSheet(ws.name) || isStatementSheet(ws.name)) continue;
		...
		db.transaction(() => {
			for (const row of dataRows(ws)) {
				...
				try {
					...
					insertPayment.run(childId, serviceId, month, year, service, unit, quantity, price, total, paid, balance, status, notes, now, now);
		...
		db.transaction(() => {
			settingsSheet.eachRow({ includeEmpty: false }, (row, r) => {
				try {
					...
					if (!label) return;
					const hourly = cellAt(row, 3);
					const monthly = cellAt(row, 5);
					if (label.includes("حضانة")) setSetting("nursery_monthly", toNum(monthly));
					else if (label.includes("استضافة")) setSetting("hosting_monthly", toNum(monthly));
					else if (label.includes("جلسة")) setSetting("session_hourly", toNum(hourly));
					else if (label.includes("نسبة الربح")) setSetting("target_profit_pct", toNum(hourly));
				} catch (err) {
...
//# sourceMappingURL=importService-DjcuiPuA.js.map