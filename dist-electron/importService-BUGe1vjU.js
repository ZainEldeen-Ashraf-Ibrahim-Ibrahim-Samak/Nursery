			...
			settingsSheet.eachRow({ includeEmpty: false }, (row, r) => {
				try {
					...
					if (label.includes("حضانة")) {
						if (monthly > 0) setSetting("nursery_monthly", monthly);
					} else if (label.includes("استضافة")) {
						if (monthly > 0) setSetting("hosting_monthly", monthly);
					} else if (label.includes("جلسة")) {
						if (hourly > 0) setSetting("session_hourly", hourly);
...
//# sourceMappingURL=importService-BUGe1vjU.js.map