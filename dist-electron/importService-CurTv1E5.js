...
async function importFromWorkbook(filePath) {
  ...
  `);
	db.prepare("SELECT id FROM child_services WHERE child_id = ? AND service = ?");
  ...
  `);
	...
	function ensureChild(name, opts = {}) {
		...
		const res = insertChild.run(name, "—", "—", null, null, opts.service || "حضانة", opts.unit || "شهر", opts.price ?? 0, opts.regDate || now.slice(0, 10), null, now, now);
	...
	if (childSheet) {
		...
		db.transaction(() => {
			for (let r = DATA_START_ROW; r <= childSheet.rowCount; r++) {
				...
				try {
					insertChild.run(name, toStr(cellAt(row, CHILD_COL.guardian)) || "—", toStr(cellAt(row, CHILD_COL.guardianPhone)) || "—", orNull(toStr(cellAt(row, CHILD_COL.childPhone))), orNull(toStr(cellAt(row, CHILD_COL.nationalId))), toStr(cellAt(row, CHILD_COL.service)) || "حضانة", toStr(cellAt(row, CHILD_COL.unit)) || "شهر", toNum(cellAt(row, CHILD_COL.price)), toStr(cellAt(row, CHILD_COL.regDate)) || now.slice(0, 10), orNull(toStr(cellAt(row, CHILD_COL.notes))), now, now);
					summary.children.imported++;
				} catch {
					summary.rowErrors++;
				}
...
//# sourceMappingURL=importService-CurTv1E5.js.map